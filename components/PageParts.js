...
export function Button({ href, outline = false, external = false, children }) { return external ? <a className={outline ? 'button button-outline' : 'button'} href={href} target="_blank" rel="noreferrer">{children}</a> : <Link className={outline ? 'button button-outline' : 'button'} href={href}>{children}</Link>; }
